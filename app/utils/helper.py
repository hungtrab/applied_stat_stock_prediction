...
def metric(confusion_matrix:np.ndarray, *, verbose = False): # 2x2 array
    ...
    precision = confusion_matrix[-1, -1] / confusion_matrix[1, :].sum() if confusion_matrix[1, :].sum() > 0 else 0
    recall = confusion_matrix[-1, -1] / confusion_matrix[:, 1].sum() if confusion_matrix[:, 1].sum() > 0 else 0
